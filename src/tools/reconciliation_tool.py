...
def _normalize_book(value: Any) -> str:
    """统一缺失账套值，避免 JE/TB 一边为空一边为默认账套导致无法匹配。"""
    if value is None:
        return '默认账套'
    s = str(value).strip()
    if s == '' or s.lower() == 'nan':
...
JE_COLUMNS = {
    ...
    "year": ["年", "年度"],
    "month": ["月", "月份"],
    "period": ["期间", "会计期间", "期间代码", "记账期间"],
    "subject": ["科目", "会计科目", "会计科目编码", "科目编码", "科目代码"],
    # 优先使用纯总账科目编码；部分序时账的“科目编码”会带客户/部门等辅助核算后缀。
    "account_code": ["会计科目编码", "科目编码", "总账科目"],
    "account_name": ["科目名称", "科目全称"],
    "debit": ["借方本位币", "借方本位币金额", "借方金额(本位币)", "借方金额(本币)", "求和项:借贷方金额(本位币)", "借方金额", "借贷方本位币"],
    "credit": ["贷方本位币", "贷方本位币金额", "贷方金额(本位币)", "贷方金额(本币)", "求和项:贷方金额(本位币)", "贷方金额"],
...
def _select_excel_sheet_and_header_row(file_path: str, prefer_sheet_keywords: Optional[List[str]] = None) -> Tuple[Optional[str], int]:
    ...
    try:
        ...
        for name in wb.sheetnames:
            ...
            for r in range(1, 11):
                ...
                if _is_balance_sheet_group_header(headers):
                    score += 20
                if any(x in header_set for x in ['期间', '月', '月份', '会计期间', '记账期间']):
                    score += 2
        ...
        if best_key is None:
            return None, 1
        return best_sheet, int(best_row)
    except Exception:
        return None, 1
        ...
        for file_path in file_list:
            ...
            if os.path.exists(file_path):
                ...
                for batch in _load_excel_in_chunks(file_path, file_type='je'):
                    ...
                    # 提取科目代码
                    if '科目' in batch.columns and '科目编码' not in batch.columns:
...
def _summarize_je_batch(je_batch: pd.DataFrame, patterns: List[str]) -> Dict[Tuple, Dict]:
    ...
    for _, row in je_batch.iterrows():
        book = _normalize_book(row.get('账套', '默认账套'))
        ...
        for file_path in file_list:
            ...
            for batch in _load_excel_in_chunks(file_path, 'je', batch_size):
                ...
                if check_voucher_balance and '凭证号' in batch.columns:
                    voucher_data = batch.copy()
                    if '账套' not in voucher_data.columns:
                        voucher_data['账套'] = '默认账套'
                    cols = ['账套', '凭证号', '借方金额', '贷方金额']
                    for optional_col in ['年', '月', '期间']:
                        if optional_col in voucher_data.columns:
                            cols.append(optional_col)
                    voucher_data = voucher_data[cols].copy()
